...
const IntelligenceSidebar = ({ item, type, onClose }) => {
  ...
  useEffect(() => {
    if (type === 'recommendation' || type === 'agent' || type === 'signal') {
      const initialText = type === 'signal' 
        ? `I am analyzing the "${item.type}" signal. Would you like to know how this macro-event specifically correlates with your enterprise KPIs?`
        : `Hello! I'm here to explain the reasoning behind this ${type === 'recommendation' ? 'decision' : 'insight'}. How can I help you understand this better?`;
      
      setChatMessages([ { role: 'ai', text: initialText } ]);
    }
  }, [type, item]);
      ...
      <div className="decision-sidebar" style={{ width: '480px' }}>
        ...
        <div className="sidebar-body">
          ...
          <div className="panel" style={{ border: '1px solid var(--border-default)' }}>
            <header className="panel-header" style={{ fontSize: '0.75rem', fontWeight: '800' }}>
              {type === 'signal' ? 'MACRO IMPACT ANALYSIS' : 'AI REASONING REPORT'}
            </header>
            <div className="panel-content" style={{ padding: '16px', fontSize: '0.85rem', lineHeight: '1.6', color: 'var(--text-secondary)' }}>
              {type === 'signal' 
                ? `Strategic Analysis: The detected "${item.event}" signal in the ${item.type} sector represents a ${item.severity} impact event. Our predictive engine has mapped this to your cost-base, suggesting that similar historical shifts have resulted in a ${item.severity === 'High' ? '5-12%' : '2-4%'} variance in operating expenses. We recommend monitoring the ${item.type} vector closely for further escalation.`
                : (item.why || detailed?.why || "Strategic Intelligence Report: This entry was flagged due to a multi-vector anomaly detection in your enterprise data. Specifically, current performance curves deviate from the 90-day moving average, implying a potential shift in underlying operational efficiency or market positioning.").replace(/\[(AUDIT|STRATEGY|GUARD)\]/gi, '').trim()
              }
            </div>
          </div>
          ...
          <div className="panel" style={{ border: '1px solid var(--border-default)' }}>
            <header className="panel-header" style={{ fontSize: '0.75rem', fontWeight: '800' }}>
              {type === 'signal' ? 'CORRELATED KPI VECTORS' : 'AFFECTED KPI METRICS'}
            </header>
            <div className="panel-content" style={{ padding: '16px' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {(item.kpis || (type === 'signal' ? ["Cost Basis", "Market Beta"] : (detailed ? ["Profit", "Margin"] : ["Enterprise Value"]))).map((kpi, idx) => (
              ...
              <div style={{ marginTop: '16px', fontSize: '0.75rem', color: type === 'signal' ? 'var(--accent-primary)' : '#52c41a', fontWeight: '700' }}>
                Projected Impact: {type === 'signal' ? `${item.severity} Volatility Expected` : (item.impact || detailed?.impact || "Optimized Performance")}
              </div>
            </div>
          </div>

          {/* Conversational AI Section */}
          {(type === 'agent' || type === 'recommendation' || type === 'signal') && (
            <div className="chat-container">
              <div className="chat-header">
                <span style={{ fontSize: '0.75rem', fontWeight: '800', textTransform: 'uppercase' }}>AI Analysis Consultation</span>
              </div>
              {/* Note: In a real app, this would use messages state, here we use chatMessages for consistency with previous edits */}
              ...
              <form onSubmit={handleSendMessage} style={{ padding: '12px', borderTop: '1px solid var(--border-default)', display: 'flex', gap: '8px', background: 'var(--bg-card)' }}>
                <input 
                  ...
                  placeholder="Ask for deeper technical reasoning..." 
